...
This section addresses the five feasibility sub-criteria defined in the
Vietnamese Student HackAIthon 2026 Bang~B (Challenger) Round~1 rubric
(\textit{Nhom tieu chi 3: Tinh kha thi} --- 25 points):
...
\begin{table}[htbp]
  ...
  information is available (see \texttt{QUESTIONS\_for\_team.md}).}
\end{table}
...
participating teams by the organiser (\textit{Ban To chuc}).
...
The team consists of four full-time undergraduate students from the Advanced
Program in Computer Science (APCS) and the High-Quality Program (CLC) of the
Faculty of Information Technology, Ho Chi Minh City University of Science
(HCMUS). Each member holds unambiguous ownership of a distinct engineering
domain:
...
operate in English, making it accessible to users in any country without
...
Asia and broader English-speaking markets as the natural second wave.
...
\begin{description}[leftmargin=1em, style=nextline, itemsep=8pt, topsep=4pt]
  ...
  \item[\textbf{Phase 1 --- HackAIthon Competition}]
    ...
    (announcement before 26~June), integrate as many VNPT API services
    as possible into the BodyCheck stack over the 8-day build window
