...
\begin{itemize}[leftmargin=2em, itemsep=2pt, topsep=4pt]
  ...
  \item \textbf{VNPT API Ecosystem (Round~2):}
        ...
        To deliver a reliable MVP within the 8-day Round~2 build window
        (26~June -- 3~July~2026), BodyCheck categorises these APIs into
        two explicit tiers:

        \textbf{Core Integration Targets (Round~2 MVP).}
        These two families are the minimum set required for the core
        triage loop and will be fully integrated and tested by the
        Round~3 submission deadline:
        \begin{itemize}[leftmargin=3em, itemsep=1pt, topsep=2pt]
          \item \textbf{SmartVoice STT/TTS (API~3.1/3.2/3.3):}
                Voice symptom intake (STT), audio response playback for
                users with accessibility needs (TTS), and multi-turn
                consultation transcript summarisation (call-summary).
          \item \textbf{Smartbot (API~4.1/4.2):}
                Deterministic rule-based first-pass triage classification
                (API~4.1) and open-ended LLM-backed differential reasoning
                and follow-up chat (API~4.2).
        \end{itemize}

        \textbf{Phase~3 / Production Roadmap Extensions.}
        The following APIs add significant value but will be integrated
        in the post-competition hardening phase once the core loop is
        stable:
        \begin{itemize}[leftmargin=3em, itemsep=1pt, topsep=2pt]
          \item SmartReader OCR and extraction (API~5.1/5.2) for
                parsing user-uploaded medical documents and lab reports.
          \item SmartUX analytics (API~7.1) for in-app interaction
                telemetry and friction-point identification.
          \item vnSocial trend listening (API~6.1/6.2) for monitoring
                health-topic sentiment on Vietnamese social media and
                surfacing epidemic signals.
          \item eKYC document OCR (API~1.1) for verified account
                creation in the B2B2C institutional channel.
          \item SmartVision face detection (API~8.3) as an optional
                telemedicine presence-confirmation extension.
        \end{itemize}

        This phased approach demonstrates realistic Round~2 scoping while
        confirming the team's commitment to leveraging the full breadth of
        the VNPT ecosystem in the production build.
\end{itemize}
...
\begin{itemize}[leftmargin=2em, itemsep=3pt, topsep=4pt]
  ...
  \item \textbf{Identity--Health Data Decoupling:}
        User identity metadata --- managed securely at the Express Gateway
        tier via JWT --- is strictly decoupled from the stateless AI
        Orchestration tier. The transient symptom payloads forwarded to
        FastAPI are completely anonymised before dispatch: all identity
        claims are stripped at the gateway boundary and replaced with an
        ephemeral, per-request correlation ID. Payloads are processed
        request-by-request without session carryover, ensuring that user
        identity can never be mapped back to health queries in any
        underlying system, log file, or audit trail.
...
BodyCheck launches as a \emph{natively bilingual platform --- English and
Vietnamese from day one}. This is not a post-MVP localisation effort: because
BodyCheck integrates VNPT's SmartVoice and Smartbot ecosystems, which are
natively optimised for the Vietnamese language, the platform provides an
immediate, high-impact localised triage solution for Vietnam's mixed
urban--rural market while simultaneously maintaining standard English
capabilities for seamless global scalability. Vietnamese-speaking users ---
including older demographics and those in rural areas with limited English
literacy --- can interact through voice (SmartVoice STT/TTS) and receive
AI-generated guidance (Smartbot) in their native language from the first
release. English-language users benefit from the same core infrastructure,
making South-East Asian, South Asian, and international expansion a natural
next step rather than a separate engineering effort.

Vietnam serves as the primary launch market given the team's local regulatory
familiarity, existing network, and the strategic depth of the VNPT ecosystem
integration. This dual-language foundation strengthens BodyCheck's alignment
with the HackAIthon 2026 organisers and VNPT as domestic technology champion.
...
\begin{description}[leftmargin=1em, style=nextline, itemsep=8pt, topsep=4pt]
  ...
  \item[\textbf{Phase 1 --- HackAIthon Competition}]
    ...
    (announcement before 26~June), integrate the Core Integration Targets
    (SmartVoice STT/TTS and Smartbot) into the BodyCheck stack over the
    8-day build window (26~June -- 3~July) with direct Mentor support.
    Demonstrate a live-running bilingual product at the Round~3 finals
    (14--15~July~2026 in Hanoi). Collect structured judge and mentor
    feedback to anchor the post-competition product direction.
  ...
  \item[\textbf{Phase 2 --- Hardening and Closed Beta}]
    ...
    schedules allow. Stabilise and document all Round~2 VNPT API
    integrations; containerise all services with a one-command
    \texttt{docker-compose} setup; implement TLS certificate
    provisioning and vault-based secret injection. Integrate the
    Phase~3 Roadmap Extension APIs (SmartReader, SmartUX, vnSocial,
    eKYC, SmartVision) during this hardening window.
    ...
    Target: a stable, security-hardened bilingual beta release by end of
    November~2026.
...
\begin{enumerate}[leftmargin=2em, itemsep=4pt, topsep=4pt]
  \item \textbf{B2C --- Bilingual Consumer Health App (Vietnamese \& English):}
        A free-tier PWA and native mobile app serving both Vietnamese and
        English-speaking users from launch. The bilingual interface --- powered
        by VNPT SmartVoice and Smartbot's native Vietnamese optimisation ---
        provides an immediate, high-impact triage solution for Vietnam's mixed
        urban--rural market while remaining accessible across South-East Asia,
        South Asia, and other regions underserved by AI-powered medical triage
        tools. Revenue via freemium subscription.
